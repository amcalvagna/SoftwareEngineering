...
public class Parameters03 {
    ...
    private static double highestGrade(List<Student> students, int year, MyPredicate<Student, Integer> filterStudents) {
        double highestScore = 0.0;

        for (Student student : students) {
            if (filterStudents.test(student, new Integer(year))) {
                if (student.getGrade() > highestScore) {
                    highestScore = student.getGrade();
                }
            }
    ...
    public static void main(String args[]) {
        ...
        double highestGrade2016 = highestGrade(students, new Predicate<Student>() {
            @Override
            public boolean test(Student student) {
                return student.getGraduationYear() == 2016;
            }
        });

        int y = 2015;
        double highestGradeCurrentAndPrevious = highestGrade(students, y, new MyFilter());

...
public class Main {
	...
	private static void query12() {
		...
		String nome = IntStream.range(1, list.size())
			...
			System.out.println(nome);		// il risultato (Optional<Libro>)
	}
	...
	private static Libro prev = null; // attributo statico per la funzione non pura
	// Funzione non pura (stato esterno)
	...
	private static void query14() {
		...
		list.forEach(x -> x.setPrezzo(
				getSconto(x.getCategoria()).apply(x.getPrezzo())));  //WOW!
